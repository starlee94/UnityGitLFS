...
public class Pokemon 
{
    ...
    public void ApplyBoost(List<StatBoost> statBoosts)
    {
        foreach(var statBoost in statBoosts)
        {
            ...
            if( boost > 0) StatusChanges.Enqueue($"{Base.Name}'s {stat} rose!");
            else StatusChanges.Enqueue($"{Base.Name}'s {stat} fell!");
    ...
    int GetStat(Stat stat)
    {
        ...
        int boostVal = StatBoosts[stat];
        var boostValue = new float[] { 1f, 1.5f, 2f, 2.5f, 3f, 3.5f, 4f};

        if (boostVal >= 0) statVal = Mathf.FloorToInt(statVal * boostValue[boostVal]);
        else statVal = Mathf.FloorToInt(statVal / boostValue[-boostVal]);
